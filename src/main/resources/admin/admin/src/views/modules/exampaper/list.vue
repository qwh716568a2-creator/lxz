...
<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '闂嵎鍚嶇О' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.exampaperName" placeholder="闂嵎鍚嶇О" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '闂嵎鐘舵€? : ''">
                        <el-select v-model="searchForm.exampaperTypes" placeholder="璇烽€夋嫨闂嵎鐘舵€?>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('exampaper','瀵煎叆瀵煎嚭')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tongyongwenjuandiaocha/upload/exampaperMuBan.xls"
                        >鎵归噺瀵煎叆闂嵎琛ㄦ暟鎹ā鏉?/a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('exampaper','瀵煎叆瀵煎嚭')"
                            ...
                            >鎵归噺瀵煎叆闂嵎琛ㄦ暟鎹?/el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 瀵煎嚭excel -->
                        <download-excel v-if="isAuth('exampaper','瀵煎叆瀵煎嚭')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "exampaper.xls">
                            <!-- 瀵煎嚭excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('exampaper','鏌ョ湅')"
                          :data="dataList"
                          v-loading="dataListLoading"
                          @selection-change="selectionChangeHandler"
                          @expand-change="expandQuestionHandler">
                        ...
                        <template slot-scope="scope">
                            <div class="question-panel">
                                <div class="question-panel__head">
                                    <strong>题目管理</strong>
                                    <div>
                                        <el-button size="mini" type="primary" icon="el-icon-plus" @click="openQuestionDialog(scope.row)">新增题目</el-button>
                                        <el-button size="mini" type="success" icon="el-icon-refresh" @click="loadQuestions(scope.row.id, true)">刷新</el-button>
                                    </div>
                                </div>
                                <el-table
                                    size="mini"
                                    border
                                    :data="questionsByPaper[scope.row.id] || []"
                                    v-loading="questionLoading[scope.row.id]">
                                    ...
                                    <el-table-column label="选项" min-width="260">
                                        <template slot-scope="questionScope">
                                            <el-tag
                                                v-for="(option, optionIndex) in parseQuestionOptions(questionScope.row.examquestionOptions)"
                                                :key="optionIndex"
                                                size="mini"
                                                class="question-option-tag">
                                    ...
                                    <el-table-column label="操作" width="220" align="center">
                                        <template slot-scope="questionScope">
                                            <el-button size="mini" type="primary" icon="el-icon-edit" @click="openQuestionDialog(scope.row, questionScope.row)">修改</el-button>
                                            <el-button size="mini" type="danger" icon="el-icon-delete" @click="deleteQuestion(scope.row.id, questionScope.row.id)">删除</el-button>
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </div>
					...
					<el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
					                  prop="exampaperJieshuyu"
					                  header-align="center"
					                  label="缁撴潫璇?>
					    <template slot-scope="scope">
					        {{scope.row.exampaperJieshuyu}}
					    </template>
					</el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="exampaperTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="鎿嶄綔">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('exampaper','鏌ョ湅')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'璇︽儏':'' }}</el-button>
                            <el-button v-if="isAuth('exampaper','淇敼')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'淇敼':'' }}</el-button>
                            <el-button v-if="isAuth('exampaper','鍒犻櫎')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'鍒犻櫎':'' }}</el-button>
                            <el-button type="warning" icon="el-icon-link" size="mini" @click="copyPublicLink(scope.row.id)">澶嶅埗鍖垮悕閾炬帴</el-button>
        ...
        <el-dialog
            :title="questionForm.id ? '修改题目' : '新增题目'"
            :visible.sync="questionDialogVisible"
            width="720px"
            append-to-body>
            <el-form ref="questionForm" :model="questionForm" :rules="questionRules" label-width="90px">
                <el-form-item label="所属问卷">
                    <el-input v-model="currentPaperName" disabled></el-input>
                </el-form-item>
                <el-form-item label="题目名称" prop="examquestionName">
                    <el-input type="textarea" :rows="3" v-model="questionForm.examquestionName" placeholder="请输入题目名称"></el-input>
                </el-form-item>
                <el-row :gutter="16">
                    <el-col :span="12">
                    ...
                    <el-col :span="12">
                        <el-form-item label="题目排序" prop="examquestionSequence">
                            <el-input v-model="questionForm.examquestionSequence" placeholder="数字越大越靠前"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
...
<script>
    ...
    //褰撳墠琛ㄤ笅鎷夋鎼滅储鏉′欢
            ...
            questionRules: {
                ...
                examquestionSequence: [
                    { required: true, message: "题目排序不能为空", trigger: "blur" }
                ]
            },
            ...
            json_fields: {
                //绾ц仈琛ㄥ瓧娈?
                //鏈〃瀛楁
                     '闂嵎鍚嶇О': "exampaperName",
                     '闂嵎鐘舵€?: "exampaperTypes",
            },
        ...
        methods: {
            ...
            expandQuestionHandler(row, expandedRows) {
                if (expandedRows.some(item => item.id === row.id)) {
                    this.loadQuestions(row.id);
                }
            },
            loadQuestions(paperId, force) {
                if (!force && this.questionsByPaper[paperId]) {
                    return;
                }
                this.$set(this.questionLoading, paperId, true);
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        this.$set(this.questionsByPaper, paperId, data.data.list || []);
                    } else {
                        this.$set(this.questionsByPaper, paperId, []);
                    }
                    this.$set(this.questionLoading, paperId, false);
                });
            },
            ...
            getNextQuestionSequence(paperId) {
                const questions = this.questionsByPaper[paperId] || [];
                if (!questions.length) {
                    return 1;
                }
                const maxSequence = Math.max.apply(null, questions.map(item => Number(item.examquestionSequence) || 0));
                return maxSequence + 1;
            },
            openQuestionDialog(paper, question) {
                this.currentPaperId = paper.id;
                this.currentPaperName = paper.exampaperName;
                ...
                if (question) {
                    this.questionForm = {
                        ...
                        examquestionOptions: question.examquestionOptions,
                        examquestionTypes: question.examquestionTypes,
                        examquestionSequence: question.examquestionSequence
                    };
                    this.questionOptions = this.parseQuestionOptions(question.examquestionOptions);
                } else {
                    this.questionForm = {
                        id: null,
                        exampaperId: paper.id,
                        ...
                        examquestionOptions: "",
                        examquestionTypes: "",
                        examquestionSequence: this.getNextQuestionSequence(paper.id)
                    };
            ...
            saveQuestion() {
                this.$refs.questionForm.validate(valid => {
                    ...
                    this.questionForm.exampaperId = this.currentPaperId;
                    this.$http({
                        ...
                        if (data && data.code === 0) {
                            ...
                            this.loadQuestions(this.currentPaperId, true);
                        } else {
                ...
                this.$confirm("确定删除该题目？", "提示", {
                    ...
                    this.$http({
                        ...
                        method: "post",
                        data: [Number(questionId)]
                    }).then(({ data }) => {
                        if (data && data.code === 0) {
                            this.$message.success("题目删除成功");
                            this.loadQuestions(paperId, true);
                        } else {
                            this.$message.error(data.msg);
                        }
                    });
            ...
            getDataList() {
				...
				if(this.$storage.get("role") != "绠＄悊鍛?){
					params['exampaperTypes'] =1
				}
                ...
                params['exampaperDelete'] = 1// 閫昏緫鍒犻櫎瀛楁 1 鏈垹闄?2 鍒犻櫎
            ...
            copyPublicLink(id) {
                const link = `${window.location.origin}/tongyongwenjuandiaocha/front/pages/exampaper/exam.html?id=${id}&anonymous=1`;
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(link).then(() => {
                        this.$message.success("鍖垮悕濉啓閾炬帴宸插鍒?);
                    });
                } else {
