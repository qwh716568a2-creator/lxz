        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '鐢ㄦ埛濮撳悕' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yonghuName" placeholder="鐢ㄦ埛濮撳悕" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '鎬у埆' : ''">
                        <el-select v-model="searchForm.sexTypes" placeholder="璇烽€夋嫨鎬у埆">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yonghu','瀵煎叆瀵煎嚭')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tongyongwenjuandiaocha/upload/yonghuMuBan.xls"
                        >鎵归噺瀵煎叆鐢ㄦ埛鏁版嵁妯℃澘</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yonghu','瀵煎叆瀵煎嚭')"
                            ...
                            >鎵归噺瀵煎叆鐢ㄦ埛鏁版嵁</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 瀵煎嚭excel -->
                        <download-excel v-if="isAuth('yonghu','瀵煎叆瀵煎嚭')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yonghu.xls">
                            <!-- 瀵煎嚭excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="鎿嶄綔">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('yonghu','鏌ョ湅')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'璇︽儏':'' }}</el-button>
                            <el-button v-if="isAuth('yonghu','淇敼')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'淇敼':'' }}</el-button>
                            <el-button v-if="isAuth('yonghu','鍒犻櫎')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'鍒犻櫎':'' }}</el-button>

                            <el-button v-if="isAuth('yonghu','淇敼')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">{{ contents.tableBtnFont == 1?'閲嶇疆瀵嗙爜':'' }}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
    ...
    //褰撳墠琛ㄤ笅鎷夋鎼滅储鏉′欢
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['yonghuDelete'] = 1// 閫昏緫鍒犻櫎瀛楁 1 鏈垹闄?2 鍒犻櫎
