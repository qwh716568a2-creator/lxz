    ...
    <div class="addEdit-block">
        ...
        >
            <el-row>
                ...
                <el-col :span="24">
                    <el-form-item class="input" style="width: 580px;" v-if="type!='info'"  label="题目名称" prop="examquestionName">
                        <el-input
                                type="textarea"
                                min="1"
                                :readonly="ro.examquestionName"
                                v-model="ruleForm.examquestionName"
                                @input="onQuestionNameInput"
                                placeholder="题目名称"
                                clearable
                        ></el-input>
                        <el-alert
                                v-if="typoIssues.length > 0"
                                class="typo-alert"
                                type="warning"
                                :closable="false"
                                show-icon>
                            <div slot="title">
                                <div class="typo-summary">检测到错别字</div>
                                <div v-for="(item,index) in typoIssues" :key="index" class="typo-issue">
                                    <span>错别字：{{item.original}}</span>
                                </div>
                            </div>
                        </el-alert>
                        <el-alert
                                v-else-if="typoCheckedText === ruleForm.examquestionName && ruleForm.examquestionName"
                                class="typo-alert"
                                type="success"
                                title="未检测到错别字"
                                :closable="false"
                    ...
                    <div v-else>
                        <el-form-item class="input" style="width: 580px;" label="题目名称" prop="examquestionName">
                            <el-input
                                    type="textarea"
                                    min="1"
                                    v-model="ruleForm.examquestionName"
                ...
                <el-col :span="24">
                    <el-form-item class="select" v-if="type!='info'"  label="题目类型" prop="examquestionTypes">
                        <el-select @change="typeChange" v-model="ruleForm.examquestionTypes" :readonly="ro.examquestionTypes" placeholder="请选择题目类型">
                            <el-option
                    ...
                    <div v-else>
                        <el-form-item class="input" label="题目类型" prop="examquestionValue">
                            <el-input v-model="ruleForm.examquestionValue"
                                      placeholder="题目类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                ...
                <el-col :span="12">
                    <el-form-item class="input" v-if="type!='info'"  label="题目排序" prop="examquestionSequence">
                        <el-input v-model="ruleForm.examquestionSequence"
                                  placeholder="题目排序" clearable  :readonly="ro.examquestionSequence"></el-input>
                    </el-form-item>
                    <div v-else>
                        <el-form-item class="input" label="题目排序" prop="examquestionSequence">
                            <el-input v-model="ruleForm.examquestionSequence"
                                      placeholder="题目排序，值越大排越前面" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    ...
                    examquestionSequence: [
                        { required: true, message: '题目排序，值越大排越前面不能为空', trigger: 'blur' },
                    ],
                }
            };
        },
        ...
        methods: {
            onQuestionNameInput() {
                this.typoIssues = [];
                this.typoCheckedText = '';
                if (this.typoTimer) {
                    clearTimeout(this.typoTimer);
                }
                this.typoTimer = setTimeout(() => {
                    this.checkQuestionTypos(false);
                }, 600);
            },
            checkQuestionTypos(showSuccess) {
                if (!this.ruleForm.examquestionName) {
                    this.typoChecking = false;
                    this.typoIssues = [];
                    this.typoCheckedText = '';
                    return Promise.resolve(false);
                }
                this.typoChecking = true;
                return this.$http({
                    url: `examquestion/checkTypos`,
                    method: "post",
                    data: { text: this.ruleForm.examquestionName }
                }).then(({ data }) => {
                    this.typoChecking = false;
                    if (data && data.code === 0) {
                        this.typoIssues = data.data || [];
                        this.typoCheckedText = this.ruleForm.examquestionName;
                        return this.typoIssues.length > 0;
                    }
                    this.$message.error(data.msg);
                    return false;
                }).catch(() => {
                    this.typoChecking = false;
                    this.$message.error("错别字检测失败，请稍后再试");
                    return false;
                });
            },
            buildTypoMessage() {
                return this.typoIssues.map(item => {
                    return `错别字：${item.original}`;
                }).join("<br>");
            },
            //选项 弹出框确定触发提交事件
            addOptions() {
            ...
            onSubmit() {
                ...
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.checkQuestionTypos(false).then(hasIssues => {
                            if (hasIssues) {
                                this.$confirm(this.buildTypoMessage(), "检测到错别字", {
            ...
            saveQuestion() {
                this.$http({
                    url:`examquestion/${!this.ruleForm.id ? "save" : "update"}`,
                    method: "post",
                    data: this.ruleForm
                }).then(({ data }) => {
                    if (data && data.code === 0) {
