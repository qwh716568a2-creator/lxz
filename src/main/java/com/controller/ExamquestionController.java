...
public class ExamquestionController {
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        if("examquestion_sequence".equals(params.get("orderBy")) && StringUtil.isEmpty(String.valueOf(params.get("order")))){
    ...
    */
    @IgnoreAuth
    @RequestMapping("/checkTypos")
    public R checkTypos(@RequestBody Map<String, String> params){
        String text = params == null ? "" : params.get("text");
        String apiUrl = environment.getProperty("typo-check.api-url", System.getenv("TYPO_CHECK_API_URL"));
        String apiKey = environment.getProperty("typo-check.api-key", System.getenv("TYPO_CHECK_API_KEY"));
        String model = environment.getProperty("typo-check.model", System.getenv("TYPO_CHECK_MODEL"));
        if(StringUtils.isBlank(apiKey)){
            apiKey = firstNotBlank(System.getenv("DOUBAO_API_KEY"), System.getenv("ARK_API_KEY"), System.getenv("TYPO_CHECK_API_KEY"));
        }
        if(StringUtils.isBlank(apiKey)){
            return R.error(500, "豆包API Key未配置，请先配置DOUBAO_API_KEY");
        }
        try {
            List<Map<String, Object>> issues = ChineseTypoChecker.check(text, apiUrl, apiKey, model);
            return R.ok().put("data", issues).put("hasIssues", !issues.isEmpty());
        } catch (Exception e) {
            logger.error("checkTypos failed", e);
            return R.error(500, "错别字检测接口调用失败：" + getRootMessage(e));
        }
    }
    ...
    private static String getRootMessage(Throwable throwable) {
        ...
        return StringUtils.defaultIfBlank(root.getMessage(), throwable.getMessage());
    }
/**
    * 删除
    */
